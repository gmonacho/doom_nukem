...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ed_display_inclined.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: gal <gal@student.42lyon.fr>                +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/05/06 14:20:16 by gal               #+#    #+#             */
/*   Updated: 2020/05/06 14:20:17 by gal              ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
const t_map *map, t_poly *poly)
{
	...
	display_line = ed_get_display_line(map,
	(t_dot){(highest.p1.x + highest.p2.x) / 2,
			(highest.p1.y + highest.p2.y) / 2},
			middle_lowest);
	ui_draw_line(win->rend, &display_line);
	circle_point = ed_get_display_point(map, middle_lowest);
	draw_circle(win, (t_circle){circle_point.x, circle_point.y,
	ed_get_line_len(&display_line) / 20});
}
